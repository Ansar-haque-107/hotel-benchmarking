...
BRAND_SCORES = {
    'Economy': 1.5, 'MidScale': 2.5, 'UpperMidScale': 3.0,
    'UpperScale': 3.5, 'UpperUpscale': 4.0, 'Luxury': 5.0,
}

AMENITIES_LIST = [
    'Daily housekeeping', 'Laundry', '24-hour front desk', 'Lift',
    'Suite Rooms', 'Bathtub in Room', 'Pet-Friendly Rooms',
    'Room service', 'Fitness center', 'Bar',
]
...
AMENITIES_KEYWORDS = {
    ...
    'Bar':                ['hotel bar', 'bar/lounge', 'cocktail bar', 'lounge bar', ' bar ', 'bartender', 'bar on site'],
}
...
def score_restaurant(combined_text):
    ...
    if has_restaurant:
        return 5.0, "On-site restaurant found -> Score: 5"
...
def score_parking(combined_text):
    ...
    if any(k in t for k in ['free parking', 'free private parking', 'complimentary parking',
                              'no charge parking', 'free self-parking', 'parking free']):
...
def process_hotel_data(hotel_input, scraped_data):
    ...
    for source_key, source_label in [
        ...
        if src:
            combined_text += ' ' + src.get('facilities_text', '')
            if src.get('rating') is not None:
                source_ratings[source_label] = src['rating']
    ...
    valid_ratings = list(source_ratings.values())
    avg_rating = round(sum(valid_ratings) / len(valid_ratings), 2) if valid_ratings else 0.0
    rating_parts = [f"{k}: {v}" for k, v in source_ratings.items()]
    ...
    for f in FACTORS:
        ...
        total += weighted
        factor_results[key] = {
            'label': f['label'], 'bucket': f['bucket'], 'weight': f['weight'],
            'score': score, 'weighted_score': weighted, 'reason': reason,
        }
...
def generate_amenities_csv(results):
    rows = [['Amenities'] + [r['name'] for r in results]]
